...
- 一覧の絞り込み・検索・並び替え・ページネーションは Go 側で処理する。
  全件を取得してブラウザ側で絞る方式は取らない。件数だけを見ればクライアント側で足りるが、
  大規模なサービスと同じ構成で作ることを目的とするため。
- 絞り込みの条件は URL の `searchParams` に持つ。ブラウザの戻る・進むが効き、リロードしても残り、URL を共有できるため。
  一覧ページは `searchParams` を読んで Go に渡し、返ってきた結果をそのまま表示する。
...
| 項目 | 内容 |
| --- | --- |
| 一覧の絞り込み・検索・並び替え・ページネーション | **必須。** `GET /records/` にクエリパラメータを追加する。絞り込みはスコア・プラットフォーム・雰囲気タグ・ジャンルの4種、タイトル検索、視聴日などでの並び替え、ページネーション。あわせて各絞り込み項目の件数（「Netflix 38件」のような表示に使う）も返す |
