...
- UI ライブラリは shadcn/ui を使う。コードを自分のリポジトリにコピーする方式なので、
  依存パッケージにならず、中身を自由に変えられるため。
- shadcn の部品は粒度に応じて `atoms/` / `molecules/` / `organisms/` に振り分ける。
  CLI の出力先は1箇所なので、部品を追加するたびに手で移動する。Atomic Design の基準に例外を作らないことを優先した。
- デザイントークンは shadcn がインストール時に `globals.css` へ書き込むものをそのまま使う。
  命名も shadcn の規約（`--background`、`--primary`、`--border` など）に従い、デザインが決まった時点で値を差し替える。
- アイコンは lucide-react。shadcn が前提にしているため。
